#
# Created By Sandy Scoloski and Nicolas Lodea
#
...
gpu$L2_Cache <- gsub("[x]", ".", gpu$L2_Cache)
gpu$ROPs <- gsub("[ ()]", "", gpu$ROPs)
gpu$ROPs <- gsub("[x]", ".", gpu$ROPs)
...
grandezas <- c("MHz", "MHz", "KB", "Watts", "MB", "GB/sec", "Bit", "MHz", "GPixel/s", "nm", "unit", "GTexel/s", "unit")
...
server <- function(input, output) {
   ...
   output$compare <- renderPlotly({
     ...
     plot_ly(x1, x = ~Name, y = x1[, input$parameter1], name = paste(input$parameter1, "(", grandezas[which(colnames(numericos) == input$parameter1)], ")"), type = 'bar', orientation = 'v') %>%
       add_trace(y = x1[, input$parameter2], name = paste(input$parameter2, "(", grandezas[which(colnames(numericos) == input$parameter2)], ")")) %>%
